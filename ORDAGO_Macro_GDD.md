...
> Versión: **1.3.2** (macro / north-star · post 2.º ciclo de diseño · 7 debates multi-IA + debate de resolución de decisiones + 2 auditorías: neuroquímica/emocional + clase-mundial/steam-premium) · Autor: César · Estado: validado por dos ciclos de debate multi-IA (7 clústeres A–G, síntesis auditada PASS anti-fabricación) + 22 decisiones de César resueltas (12 consenso 3/3, 5 mayoría 2/3, 2 experimento) + 2 auditorías ancladas a cartas del brain (lente venta premium Steam, filtro de modelo LIMPIO sin trampas F2P); listo para micro-GDD + solver de papel
...
> ### 🔄 Qué cambió en v1.3.2 (resumen ejecutivo)
> La v1.3.1 fue sometida a **2 auditorías ancladas a cartas verificadas del brain** con la lente **venta premium en Steam (B2P, sin MTX)**: (a) una **neuroquímica/emocional** y (b) una **clase-mundial/steam-premium**. Patrón único: cerrar los **dos residuos** del mismo "máquina sin cablear" que el proyecto juró matar — (1) **4 sistemas EMOCIONALES** que seguían sin **gate falsable** (lazo D1/D7, game-feel, arco de Sospecha, sesión-maratón) y (2) la **capa COMERCIAL** menos cableada por el ciclo de diseño: horizonte de ingreso a años (cola larga), el **puente de COMPRA** móvil→PC, y el **ritual GRUPAL** que mantiene el grafo caliente sin live-ops.
> Ambas auditorías **confirmaron clase mundial en todo lo demás** y, crucialmente, el **filtro de modelo salió LIMPIO (cero trampas F2P)** — se mantienen las zonas de vigilancia (D1/D7 y k-factor como vecinos del growth F2P: su gate refuerza "proxy de viralidad/Playtime-Depth, **no** retención-para-monetizar").
> Todo es **adición/instrumentación**, no cambio de cimiento: cada sistema emocional pasa a invariante V/🟡/KILL; la capa comercial gana §16.3 (cola larga), gate de puente de compra, ritual grupal, segunda pantalla y guardarraíles de keys/precio. Changelog maestro y fidelidad anti-fabricación: `docs/research/2026-06-26-ordago-auditoria-clasemundial/CIERRE-v1.3.2.md`.
...
> **Pity-timer invisible (NUEVO — v1.3.2, auditoría emocional A-P2, 05-schedules 🟡 "pity timers invisibles").** El botín de la Cantina lleva **bad-luck-protection numerada e invisible**: tras una racha de rerolls/ofertas pobres, la probabilidad de un ítem relevante a la build sube de forma garantizada hasta tope, sin que el jugador vea el contador (se siente como suerte, no como sistema). Evita la espiral de frustración por varianza adversa sin convertirlo en un mecanismo visible de gacha. **El principio se declara aquí; los números viven en el micro-GDD** (§14).
...
- **Endowed Progress Effect (NUEVO — v1.3.2, auditoría emocional A-P2, 06-loss-aversion 🟡):** las barras de progreso del **Codex y la Codicia arrancan con casillas ya regaladas, NUNCA en 0/N** ("nunca una barra en 0"). El jugador entra con una sensación de avance ya iniciado (p.ej. el Codex muestra `2/N` desbloqueado de salida; la primera Codicia ya tiene su primer escalón "abierto por la casa"). Adición barata, alto retorno en motivación de completar.
...
- **Autonomía social opt-in (NUEVO — v1.3.2, auditoría emocional A-P2, SDT 02 🟡):** el jugador **elige entre 2-3** qué semilla / qué Diablo del grafo enfrentar, en vez de recibir uno impuesto — elección significativa (autonomía SDT), **sin romper** "0 sintéticos / una escalera real": los 2-3 candidatos salen del **mismo bucket cerrado de pares reales** y todos cuentan igual en la tabla comparable. No es FOMO ni ratchet; es darle volante a la confrontación.
...
- **Ritual GRUPAL — la cita diaria como evento de grupo (NUEVO — v1.3.2, auditoría clase-mundial B-P2):** la seed del día no es solo un daily solitario ni un 1-vs-1: es una **cita grupal**. Incluye un **leaderboard DE CONTACTOS** (tu grafo, no solo global ni el Diablo Fantasma individual), un botón **"Reta a tu grupo"** de **primera clase** (no enterrado), y una **ventana ritual a hora pico** que sincroniza al grupo. **Para un premium sin live-ops es el retorno D7+ más barato** que mantiene el grafo caliente entre olas de discovery (alimenta §16.3 cola larga, bajo el techo del gobernador anti-spam §18.1 y los guardarraíles G).
- **Dos planos de tabla:** canónica (intacta) + **"Retos del Romancero"** (tablas ortogonales auto-verificadas por seed, C3).
- **Autonomía social opt-in en la Cantina (v1.3.2, A-P2, SDT 02 🟡):** el jugador **cura qué trofeo diegético exhibe** la Cantina del Tahúr (§8.4) — expresión-no-poder, elección significativa sin tocar la pureza de la run ni la escalera real.
...
- **Puente de continuidad móvil→PC — el handoff de COMPRA (NUEVO — v1.3.2, auditoría clase-mundial B-P1):** el grafo vive en **móvil** (WhatsApp), pero la venta vive en **PC** (Steam). El artefacto-puente encendía el VERBO pero no cerraba la **compra cross-device**. Se diseña el handoff explícito:
  - **Wishlist-en-un-tap desde móvil** (Steam app / web móvil) al terminar la demo — sin fricción de teclear.
  - **QR a PC** en la **pantalla de victoria de la demo** ("domínalo entero en tu PC") → puente físico móvil→escritorio.
  - **Email opt-in día 1** → pre-registro / aviso de lanzamiento (red de seguridad para el jugador móvil-sin-PC, ver §15 LATAM ~25% ⚪).
  - **Explícito (doctrina):** *"el grafo LATAM existe para **encender a hispanos-USA / USA**, no para convertirse a sí mismo"* — la viralidad LATAM es motor de alcance que prende los segmentos de mayor LTV, no se le pide que cierre la venta en su propio mercado.
...
- **Constraint-flags + sello de reto compartible (NUEVO — v1.3.2, auditoría clase-mundial B-P3, C3 ⚪ — el motor "se juega 5 años"):** el `RunDigest` incorpora **`constraint_flags` serializables y verificables por seed** — gané con **0 Fullerías** / **Escoba perfecta** / **sin corromper ningún Pacto** / **speedrun de la Manga 4**, etc. Cuando una run cumple un constraint, el **sticker/clip estampa un sello visual de reto** (legible al glance, no número). Más **3–5 retos oficiales de semilla del estudio**: el metagame de auto-imposición de restricciones **raramente surge solo**; se enciende con un ejemplo curado. Código barato sobre la infra de `RunDigest` y verificación-por-seed ya existente; es el anclaje de longevidad comunitaria (5 años).
...
- **"El Diablo de hoy":** reto diario con sabor (Trampa rara + Pacto), trofeo no-ranking. **No solo solitario (v1.3.2):** se acompaña del **ritual GRUPAL** — leaderboard de contactos + "Reta a tu grupo" + ventana a hora pico (§8.2) — el vehículo de retorno D7+ más barato para un premium sin live-ops.
...
> **Gate del lazo D1/D7 (NUEVO — v1.3.2, auditoría emocional A-P1).** Cierra el único gran sistema emocional que quedaba sin gate falsable. Umbral V/🟡/KILL sobre `% retorno D7 atribuible a (trofeo de Cantina vs daily vs artefacto-puente)`, **segmentado por clúster QF**:
> - **VERDE:** ≥1 de los 3 vehículos supera su piso de atribución D7 con señal limpia por clúster.
> - **🟡:** atribución difusa entre los 3 (ninguno claramente dominante) → re-instrumentar la atribución antes de invertir más.
> - **KILL:** ningún vehículo mueve el retorno D7 sobre baseline → el lazo no existe; archivar la Cantina del Tahúr como adorno, no como sistema.
> **Nota de aplicabilidad premium (zona de vigilancia, filtro F2P LIMPIO):** en un **B2P sin live-ops**, el D1/D7 es **proxy de viralidad / Playtime-Depth (profundidad de juego que alimenta reviews y reenvío), NO un KPI de retención-para-monetizar.** Prohibido por doctrina convertir este gate en un loop de re-enganche diario monetizado; su único propósito es mantener el grafo vivo entre olas de discovery (ver §16.3 cola larga, §8.2 ritual grupal).
...
> **Centinela de fatiga del arco de Sospecha (NUEVO — v1.3.2, auditoría emocional A-P1).** La telemetría de fatiga genérica de §18 no medía la **fatiga emocional por sobre-tensión** del duelo de pillado (§7.6b). Se añade un KPI-centinela específico atado a **gate de release**: `abandono tras N pillados seguidos` (y/o `re-run<3s tras pillado`). Si la sobre-tensión del arco de Sospecha dispara abandono por encima del umbral calibrado → la tensión está mal dosificada (el near-miss no alivia, el castigo tiltea) → **revisar la curva de Sospecha antes de shippear**. Es el gate emocional gemelo del centinela de uso compulsivo (§18.1).
...
> **Los 3 tests de LD7 nombrados explícitamente (v1.3.2, auditoría emocional A-P2 — trazabilidad ética).** Junto a los gates D30, toda feature de meta/colección/social debe pasar la aserción LD7 (perdurabilidad ética sin compulsión), declarada con sus tres ejes:
> 1. **Terminabilidad** — la colección/sistema **se puede completar** y al completarse **se silencia** (sin infinitud manufacturada). Ya verificado en Codex terminable (§8) y en el Codex de Arrepentimientos (`has_experienced_regret` permanente).
> 2. **Saciedad** — existe un estado "satisfecho" alcanzable; el KPI-centinela "para satisfecho vs encadena" (§18) y el ratio de saciedad social (§18.1) lo miden.
> 3. **Proporción** — la recompensa es **proporcional al esfuerzo**, sin ratchet ni FOMO; los desbloqueos dan autonomía, no recompensa contingente (anti-overjustification, §6.4).
> Estos tres son aserción **explícita** de release alongside los gates D30 — no buena intención.
...
> **Gate de game-feel — U-invertida de Kao (NUEVO — v1.3.2, auditoría emocional A-P1).** El juice deja de ser "más es mejor" y pasa a invariante falsable, análogo al `C ≤12/mano` cognitivo:
> - **A/B `juice medio` vs `juice extremo`** (replica Kao 2020) sobre **tiempo-de-juego y motivación intrínseca**. El ganador es el pico de la U-invertida, no el extremo.
> - **Presupuesto numérico `juice/segundo`** como techo duro de feel (análogo al `C ≤12/mano`): la densidad de eventos de juice por segundo no debe superar el techo calibrado; las cascadas coalescen (crescendo único) en vez de N golpes independientes.
> - **KILL si** el juice cae en la zona "no-juice" (motivación intrínseca ≤ baseline sin feedback) **o** en la zona "Extremo que marea" (tiempo-de-juego/motivación caen vs el medio). El Extremo no se shippea aunque "se vea impresionante".
...
> **Renovación de awe post-launch sin live-ops (NUEVO — v1.3.2, auditoría clase-mundial B-P3, A2).** Los **updates gratis estilo "Friends of Jimbo"** (§16, §16.3) funcionan como **2.ª ola de reviews / re-evangelismo** — renuevan el asombro sin convertirse en live-ops (respetan el cap G3 ≤0.8 d-h/dosis y el principio "share, no retención"). Además se **nombra explícitamente el awe del trailer/capsule**: el **primer money-shot que ve el comprador** NO es uno in-game sino el del **trailer + capsule de Steam** — debe pasar el mismo listón de "reacción muda" (G5 ≥70% en <5 palabras) porque es la superficie que decide la compra. Alimenta la cola larga (§16.3).
...
- **Glance-gate del capsule de Steam (NUEVO — v1.3.2, auditoría clase-mundial B-P3, C1):** el **capsule canónico** pasa el **mismo gate que el sticker** (§8.3): legible **≥90% @ 100×100 px en gris y mudo** (juez nuevo nombra el gancho del juego <0.5 s), **KILL <90%**. Es la **superficie de conversión clip→wishlist más cara de un premium** (el frame que el comprador ve en la tienda); merece el rigor del glance-gate, no solo el ojo del artista.
...
> **Style Bible de 1 página (NUEVO — v1.3.2, auditoría clase-mundial B-P3, A5).** ANTES del primer asset generado por IA, fijar una **Biblia de Estilo de 1 página** con: **shape language** (lenguaje de formas) + **color script** (hex + función de cada color, no solo paleta bonita) + **regla de tono** + lista **"qué sí / qué no"**. Razón: en un estudio chico, **reescribir la dirección de arte a mitad del pipeline es el desperdicio más caro** — la 1 página es el ancla barata que evita el drift. Alimenta el A/B de §11 (los lados A/B deben declararse contra esta página) y el capsule de Steam.
...
- **Moderación cultural — `authenticity_flags` (NUEVO — v1.3.2, auditoría clase-mundial B-P3, B3).** Con ~12–20 Diablos del Romancero estilo **Posada**, existe riesgo de **deslizamiento a caricatura** sin un instrumento de detección. Se añaden **`authenticity_flags` por arquetipo** (marcan cada Diablo/Trampa/Fullería con su nivel de riesgo de estereotipo/apropiación) + una **consulta de nodo cultural formalizada pre-1.0** (consultores del nodo, ver §11 multi-nodo C-3). El objetivo es distinguir **homenaje folk auténtico** de **caricatura**, con un flag medible, no con el buen juicio del momento. Cruza con el riesgo IP/blanqueamiento/code-switching de §11 y §19.1.
...
> **Endowed Progress en el onboarding (v1.3.2, A-P2):** las barras de progreso que el jugador encuentra en sus primeras sesiones (Codex, Codicia — §8) arrancan **regaladas, nunca en 0/N** (06-loss-aversion). El nuevo jugador percibe avance ya iniciado desde la 1.ª victoria, lo que aumenta la probabilidad de perseguir el cierre.
...
- **Gate del puente móvil→PC (NUEVO — v1.3.2, auditoría clase-mundial B-P1, ⚪→falsable):** cierra el residuo de que la conversión cross-device era ⚪ sin VERDE/KILL. Medir en **cohorte** `demo-móvil → wishlist` (vía wishlist-en-un-tap / QR / email, §8.3): hoy estimado **12–18% (⚪)**. **Pasa si** la cohorte supera el piso calibrado de wishlist desde demo móvil; **🟡** en banda baja → priorizar la fricción del handoff (QR/un-tap); **revisar la viabilidad del canal móvil-primario** si la conversión cae muy por debajo. El número exacto del piso lo zanja el dato de Next Fest (cruza con DC-15, [experimento]).
...
- **Segunda pantalla — guías curadas en YouTube ES (NUEVO — v1.3.2, auditoría clase-mundial B-P2, S5 🟡):** sistema de **contenido curado por el estudio en YouTube en español** (sinergias / builds / Diablos del Romancero) — **NO es thin-content** como el SEO-Engine por-seed diferido: es contenido humano de valor que alimenta el long-tail orgánico (**ratio ~3:1 búsqueda:clic-directo**, S5) con **CAC≈0**. La señal de Search Velocity (centinela ya existente arriba) mide el efecto; se añade el **motor de contenido curado** como palanca explícita de descubrimiento. Vive junto a la matriz de UA (§17).

### 16.3 Modelo de ingreso a horizonte de años — la cola larga (NUEVO — v1.3.2, auditoría clase-mundial B-P1, S10 🟡)
La auditoría de venta premium en Steam expuso el residuo comercial mayor: el GDD modelaba el **pico de lanzamiento** pero no el **horizonte de años**. Para un premium B2P, **~la mitad del ingreso llega DESPUÉS del mes 1** (cola larga, S10). El north-star comercial deja de ser el pico de launch y pasa a ser el **ingreso acumulado por trimestre**.

- **Calendario de cola larga (ético, sin MTX, sin live-ops):**
  1. **Launch −20%** ($11.99 sobre $14.99; regional $7.49 LATAM) — el descuento de lanzamiento canónico (§16.2).
  2. **Seasonal sales ≥20% espaciadas**, **sincronizadas con las sales mayores de Steam** (Summer/Autumn/Winter/Spring) — cada sale = nuevo pico de visibilidad + cohorte de wishlist-a-venta.
  3. **DLC / update reactivador post 9–12 m** (ético, **sin MTX**: contenido o Tahúr/Codicia nuevos, estilo "Friends of Jimbo") → **2.ª ola de reviews** y re-evangelismo (ver §10.2 awe post-launch).
- **KPI:** **ingreso acumulado por trimestre**, NO solo el pico de lanzamiento. Se reporta por columna (USA / hispanos-USA / LATAM, §16.1) para ver qué segmento sostiene la cola.
- **Zona de vigilancia (filtro F2P LIMPIO):** la reactivación es por **valor entregado** (contenido gratis o DLC honesto), nunca por mecánicas de re-enganche monetizado; el lazo D1/D7 (§8.4) y el ritual grupal (§8.2) mantienen el grafo caliente **entre** olas, no lo monetizan.
...
> **Segunda pantalla — guías curadas YouTube ES (v1.3.2, B-P2, S5):** motor de **contenido curado en español** (sinergias/builds/Diablos) como long-tail orgánico CAC≈0 (ratio ~3:1 búsqueda:clic-directo). NO es thin-content (≠ SEO-Engine por-seed diferido). Detalle y gate de Search Velocity en §16.2.

> **Precio FUERA del creativo (NUEVO — v1.3.2, auditoría clase-mundial B-P2, S7):** regla dura de la matriz de UA — **nunca el número de precio en el hook/overlay** de un creativo. El USD literal en LATAM cuesta **−28% CTR**. El precio vive en la página de Steam, jamás en el frame que enciende el reenvío. Se hereda a TODA variante de la fábrica de creativos.

> **NUNCA #11 — anti-reventa de keys (v1.3.2, B-P2, S9 → §19.5):** no emitir Steam Keys con descuento regional a terceros / bundles / prensa. Humble/Fanatical y similares se sirven **global a precio completo** (mecanismo anti-arbitraje gris). Invariante de release, no recomendación (§19.5 #11).
...
- **Fatiga emocional del arco de Sospecha (v1.3.2):** `abandono tras N pillados seguidos` / `re-run<3s tras pillado` — gate de release (§9). Distinto de la fatiga genérica: mide sobre-tensión del duelo de pillado, no cansancio de sesión.
- **Uso compulsivo por tiempo (v1.3.2):** sesiones-maratón (N runs seguidas) y disparo del nudge diegético de pausa — KPI-centinela ético (§18.1), zona de vigilancia F2P.
...
- **Gate de sesión-maratón — vulnerabilidad adulta por tiempo (NUEVO — v1.3.2, auditoría emocional A-P1):** el único eje de LD7/12 que seguía sin invariante. **Nudge diegético tras N runs seguidas** (*"el Diablo te ofrece una pausa, tahúr"* — opt-in, sin penalización, sin interrumpir el flow a media run) + **KPI de uso compulsivo por tiempo** (distribución de duración de sesión, cola de sesiones-maratón). En B2P sin live-ops el objetivo es **bienestar, NUNCA retención-para-monetizar**: el nudge protege al jugador, no lo re-engancha (zona de vigilancia F2P, filtro LIMPIO). Refuerza el guardarraíl §19.5 #11. *(Umbral N y forma del nudge = micro-GDD / decisión de César; aquí se declara el invariante y su dirección ética.)*
...
| **Modelar solo el pico de launch e ignorar la cola larga (~½ del ingreso post-mes-1)** | Media (v1.3.2) | §16.3: calendario de cola larga (seasonal sales + DLC reactivador), KPI = ingreso acumulado por trimestre |
| **El grafo móvil (WhatsApp) no convierte a compra en PC (Steam)** | Media (v1.3.2) | §8.3 puente de compra (wishlist-un-tap / QR / email) + gate falsable demo-móvil→wishlist (§14) |
| **Sesión-maratón / uso compulsivo por tiempo (vulnerabilidad adulta)** | Baja-Media (v1.3.2) | §18.1 nudge diegético de pausa + KPI de uso compulsivo; §19.5 #12 (bienestar, no re-enganche) |
...
### 19.5 Guardarraíles éticos — los "NUNCA" (NUEVO — v1.3, G + D, inviolables por código · ampliado en v1.3.2)
> De G (3 fixes rechazados + corolarios), reforzado por D **y, en v1.3.2, por las 2 auditorías (#11 anti-reventa de keys S9; #12 sesión-maratón)**. **No son recomendaciones: son invariantes de compilación/release.**
...
11. **NUNCA emitir Steam Keys con descuento regional a terceros / bundles / prensa** (v1.3.2, S9). Las keys a Humble/Fanatical/etc. van **global a precio completo** — mecanismo anti-arbitraje gris de reventa (anti-reventa de keys, §17).
12. **NUNCA convertir el nudge de sesión-maratón en re-enganche monetizado** (v1.3.2). Tras N runs seguidas, el nudge diegético de pausa ("el Diablo te ofrece una pausa, tahúr") existe para **bienestar del jugador**, jamás para retención-para-monetizar; el KPI de uso compulsivo por tiempo se mide para proteger, no para optimizar engagement (§18.1).

---

*Fin del macro GDD v1.3.2. Siguiente paso lógico: (1) el **solver de papel** del generador de bifurcación con los pasos **S4–S7** (§14.0) — valida o mata el foso, la dominancia de Maña y el escalado del umbral del Diablo por un finde, ANTES de un píxel; (2) el **micro-GDD del prototipo** — las ~3 Trampas + 3 Fullerías + 3 Pactos concretos con números, las reglas de siembra de la Mesa viva, las matas con sus valores, los umbrales de la primera Manga, y el spec del `RunDigest` → ficha-sticker autocontenida (`grito_glifo`, `text_glyph`) + emisor `emitStateCue(visual, audio)`. Plan maestro de este ciclo: `docs/research/2026-06-25-ordago-debate-diseno-v2/META-SINTESIS-v1.3.md`.*
