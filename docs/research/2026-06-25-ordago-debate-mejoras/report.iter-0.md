...
- **Objeto A — La ficha-sticker (PRIMARIA, la que cruza el grafo familiar).** Imagen **PNG/WebP <80 KB renderizada EN CLIENTE** con sprites pre-cargados (0 servidor, 0 dependencia de red para generarla). Muestra: la "Mano contra el Diablo" + la **línea-verbo-nominal** *"Le robé el alma a @primo por +X%"* + score + la cara del Diablo en tipografía Posada.
  ...
  - **Opus concedió en R3** ("imagen primaria; perdí el formato, no el principio") — su argumento de "0 servidor" se preserva porque el PNG se renderiza en cliente.
...
Deep-link jugable `ordago.gg/d/<seedhash>` **embebido en el caption de ambos objetos**. Razón: comprensión sin acción es K de un solo salto. El receptor necesita QUÉ hacer → juega la mano → genera SU sticker → reenvía. Eso **compone** K (K = i × c, C2 🟡). La imagen gana la primera `i`; el deep-link gana la segunda. *Detalle de física de plataforma (Gemini, L1):* los stickers de WhatsApp no admiten hipervínculos, pero el menú nativo de compartir permite imagen + caption con el link → por eso imagen primaria **+ caption de texto** que contiene la línea-verbo y el link (el texto plano sobrevive como fallback universal si la imagen no carga).

### El doble output — resuelto: separar GENERAR de CODIFICAR (aporte de síntesis de Opus, R3)
El crux "clip móvil vs solo-PC" se **disolvió** (Contestado R2 → Consenso R3):
- El cliente móvil **NO codifica video** (el hardware medio-bajo LATAM con MediaTek/Snapdragon antiguos chokea el main thread y agota el heap del navegador — data L1 dura, Gemini+Meta; **87% de gamers LATAM en smartphone gama media-baja**, Meta).
- **El clip se genera en PC/Steam o vía render-worker server-side** desde el `RunDigest`; el móvil **recibe y reenvía** un asset pre-renderizado ligero (MP4/WebP <500 KB), no lo genera. Así el móvil preserva fricción-cero (B5 🟢) y el alcance broadcast no se amputa.
...
- **Loop de ADQUISICIÓN (CAC > 0 SIEMPRE):** clip broadcast → demo web → wishlist Steam → compra. El "CAC~0 vía WhatsApp" es **físicamente imposible** si el link aterriza en un paywall de $15-20. **Borrar "CAC~0" del lenguaje de adquisición** (corrección editorial gratis). Aquí vive el costo del render server-side del clip.
...
> *Las cifras de CAC/LTV/conversión que cada IA puso en sus tablas (p.ej. Gemini LTV/CAC 2.84/4.87/10.80; Meta 18%/15%/12% de conversión) NO son consenso y NO se elevan a hecho. Son ilustraciones de industria/razonamiento. El entregable falsable es el modelo de payback escrito, validado en cohorte propia.*
...
|---|---|---|---|---|---|
| **§8.3 nuevo** | Crear "El artefacto-puente": sistema dual `RunDigest` → (A) sticker-imagen PNG/WebP <80KB en cliente + caption con verbo-nominal y deep-link; (B) clip 6-8s <500KB con audio, render server-side/PC | 1 | C2, D1, A3 | 🟡 (L1 formato) | A/B 20 grupos MX: kill si reenvío 24h <30% **o** comprensión <2s <70% |
